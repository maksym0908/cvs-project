...
import classes from './FilterByDisabilityJobs.module.scss'


export const FilterByDisabilityJobs = props => { 
    ...
    return (
        <div  className={classes.container}>
            ...
            <h5 className={classes.text}><b>Вакансии по категориям для людей с инвалидностью</b></h5>
            ...
            <ul  className={classes.categoriesWrapper}>
                {categories.map(c => (
                    <li>
                        ...
                        : '?category=' + c.value}` + "&by-disability=1&page=1" }>
                            {c.label}
                        </Link>
                        <span>
                            {vacanciesCategories.filter(v => v.category === c.value && v.another === "disability").length}
                        </span>
                    </li>
                ))}
            </ul>
