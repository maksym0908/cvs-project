...
import classes from './Filter_jobs_for_students.module.scss'

export const FilterByJobsForStudents = props => {
    ...
    return (
        <div  className={classes.container}>
            ...
            <h4 className={classes.text}><b>Поиск вакансий для студентов по категориям</b></h4>
            ...
            <ul  className={classes.categoriesWrapper}>
                {categories.map(c => (
                    <li>
                        <Link to={`/vacancies${cityValue ? `?city=${cityValue}&category=${c.value}` : `?category=${c.value}`}&stud=1&page=1` }>
                            {c.label}
                        </Link>
                        <span>
                            {vacanciesCategories.filter(v => v.category === c.value && v.another === "student").length}
                        </span>
                    </li>
                ))}
            </ul>
